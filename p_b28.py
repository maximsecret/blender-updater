...
def b_create():
	os.system("cd " + b_path + " && git clone -b blender2.8 git://git.blender.org/blender.git blender28 && cd blender28 && git submodule update --init --recursive && git submodule foreach git checkout master && git submodule foreach git pull --rebase origin master")
...
def b_pull():
	...
	os.system("cd " + b_path + "/blender28 && make update")
	print("----------------BLENDER_PULL_END----------------")
...
def b1():
	...
	if not os.path.exists(b_path+'/BlenderMyChanges'):
		...
		os.system("cd " + b_path + "/build && rm -rf * && cmake "+b_flags+" -DCMAKE_CXX_FLAGS_RELEASE=\"" + c_flags +"\" -DCMAKE_C_FLAGS_RELEASE=\"" + c_flags +"\" -DCMAKE_INSTALL_PREFIX=\"" + i_path + "blender28/\" ../blender28/")
	else:
		print("With folder "+ b_path+'/BlenderMyChanges')
		if not os.path.exists(b_path+'/tmp_blender'):
			os.makedirs(b_path+'/tmp_blender')
		os.system("cd " + b_path + "/ && rm -rf tmp_blender/* && cp -R blender28/* tmp_blender/ && cp -R BlenderMyChanges/* tmp_blender/")
		os.system("cd " + b_path + "/build && rm -rf * && cmake "+b_flags+" -DCMAKE_CXX_FLAGS_RELEASE=\"" + c_flags +"\" -DCMAKE_C_FLAGS_RELEASE=\"" + c_flags +"\" -DCMAKE_INSTALL_PREFIX=\"" + i_path + "blender28/\" ../tmp_blender/")
